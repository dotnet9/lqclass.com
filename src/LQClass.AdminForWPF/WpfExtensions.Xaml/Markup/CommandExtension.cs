...
public class CommandExtension : MarkupExtension, ICommand
{
    ...
    public bool CanExecute(object parameter)
    {
        if (_canExecute == null)
        {
            var context = _frameworkElement?.DataContext;
            ...
            if (parameters.Length > 1)
                throw new InvalidOperationException(
                    $"The method named \"{methodName}\" must only have 0 or 1 parameters.");
    ...
    public void Execute(object parameter)
    {
        if (_execute == null)
        {
            ...
            _execute = async args =>
            {
                ...
                var result = method.Invoke(context, hasParameter ? new[] { args } : null);
                if (isTask)
                {
                    _isExecuting = true;
